...
class TestOAEP < Test::Unit::TestCase
  ...
  def test_oaep
    ...
    section = title = nil
    key = nil
    n = e = d = p = q = dp = dq = qinv = msg = seed = cipher = nil
    @content.each_line do |line|
      case line
      when /^# Example/
        section = line.chomp
      when /Encryption Example/
      ...
      when /^\s*$/
        ...
        if check
          do_check(title, key, msg, seed, cipher)
          check = false
        end
      end
    end
  ...
  def do_check(title, key, msg, seed, cipher)
    puts title
    msg = hex2b(msg)
    seed = hex2b(seed)
    cipher = hex2b(cipher)
    encryptor = PKCS1::EncryptionScheme::RSAESOAEP.new(Digest::SHA1)
    mycipher = encryptor.encrypt(key, msg, seed)
    assert_equal(cipher, mycipher, title)
  end
